...
> 暂未实现。下文已对**每一条**架构风险/测试盲区标注状态（✅ 已修复 / 🟡 部分 / ⏳ 未处理）。
> 本轮已修复的本报告条目：
>
> - 第 4 节架构风险：**M1**（async 计数器无锁撕裂读）、**M5**（轮转清理 OOM 静默）、
>   **M6**（`sigaltstack` 失败仍设 `SA_ONSTACK`）、**L9**（`format_with_retry` 用 `strlen` 估算）；
> - 第 5 节测试盲区：**T2**（多线程并发写入）🟡 部分覆盖（新增并发 reconfigure 压测，CI TSan 运行）。
>
> 其余条目 ⏳ 未处理：高优先级功能缺口（编译期级别裁剪、TCP/UDP sink、文件/环境配置、Logger 层级继承）、
> M2/M3/M4/M7/M8、L1–L8/L10–L16、以及其余测试盲区。完整提交/CI 信息见
> [发布完备性审计报告 · 第 0 节 整改状态](2026-06-19-hx-clog-v1.2.0-completeness-audit.md#0-整改状态2026-06-19-更新)。
...
> 整改：✅ 已修复 3 项（M1、M5、M6） / ⏳ 未处理 5 项（M2、M3、M4、M7、M8）。

**M1. 异步 drop 计数器数据竞争（32 位平台撕裂读）** — ✅ **已修复**（`6aa26c1`，对应发布审计 M-03）：`hx_async_dropped()`/`hx_async_high_watermark()` 现在在 `g_async_lock` 下读取。
...
**M3. 非 GCC/non-MSVC 编译器 TLS 静默退化为全局变量** — ⏳ **未处理**
...
**M4. Ring Buffer 初始化 OOM 静默 — 所有 ring push 变为空操作** — ⏳ **未处理**（注意：与已修复的 M5 不同，本项是崩溃 ring buffer 的 *init* OOM，仍未上报）
...
**M5. 轮转清理 OOM 静默跳过 — 备份清理被放弃** — ✅ **已修复**（`6aa26c1`）：`hx_rotate_cleanup()` 的两条 OOM 路径现在都调用 `hx_core_report_error()`。
...
**M6. `sigaltstack` 失败未处理 — 仍带 `SA_ONSTACK` 安装崩溃处理器** — ✅ **已修复**（`6aa26c1`，对应发布审计 M-07）：检查 `sigaltstack`/`sigaction` 返回值，仅在 `sigaltstack` 成功时才设 `SA_ONSTACK`，失败均上报。
...
**M7. 异步批量写期间移除 Callback Sink — 悬垂指针** — ⏳ **未处理**（M-02 的递归锁修复的是回调*重入*死锁，与本项 worker 持悬垂指针无关）
...
**M8. pkg-config `Libs.private` 在 ARM32/MIPS 上缺少 `-latomic`** — ⏳ **未处理**
...
> 整改：本组仅 **L9** 因 M-05 重写连带解决（见下表标注）；其余 15 项 ⏳ 未处理。
...
| L9 ✅ | `core.c` | ✅ **已修复**（`6aa26c1`/M-05）：`format_with_retry` 改用格式化返回的*所需长度*精确扩容，不再用 `strlen()`/启发式估算 |
...
**T2. 多线程并发写入 — 零覆盖** — 🟡 **部分覆盖**（`6aa26c1`）：`tests/test_audit_fixes.c` 的 `test_async_reconfigure_no_loss` 用 4 个并发生产者 + 并发 `reconfigure` 压测异步队列，CI 在 TSan 下运行。仍缺 DROP_OLD（T1）和纯并发吞吐压测。
- 所有测试都是单线程、单生产者。异步队列在并发生产者下的表现 — 设计的全部目的 — 从未被验证。mutex/condvar 原语仅在平凡的单生产者单 worker 模式下使用。~~**零测试。**~~（已部分覆盖，见上）
